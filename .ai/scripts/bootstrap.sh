...
[[ "$compose_version" == 2.* ]] || { echo 'Docker Compose v2 is required.' >&2; exit 1; }
...
python3 - "$root/.ai/tooling/language-servers/package-lock.json" <<'PY'
import json, pathlib, sys
value = json.loads(pathlib.Path(sys.argv[1]).read_text())
assert value["lockfileVersion"] == 3
assert value["packages"][""]["dependencies"] == {"bash-language-server": "5.6.0", "pyright": "1.1.403"}
assert all("integrity" in entry for path, entry in value["packages"].items() if path)
PY
...
[[ "$after_index" == "$before_index" ]] || { echo 'Git index changed during the project-local build.' >&2; exit 1; }
echo 'Project-local AI image build completed without starting services or mutating the Git index.'
