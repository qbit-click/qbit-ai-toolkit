...
The launcher remains portable because workspace root, hostname, tunnel name, and host-execution mode are supplied at runtime.
...
Verify that the public hostname routes to the local CodexPro endpoint and that unauthenticated requests cannot gain MCP access.
