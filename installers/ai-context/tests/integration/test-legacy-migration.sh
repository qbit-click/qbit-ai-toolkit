...
PY
  ...
  [[ "$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["installerVersion"])' "$state")" == 1.2.2 ]]
...
run_test 'fresh member seed survives uninstall and version-only update reaches 1.2.2' t_fresh_header
