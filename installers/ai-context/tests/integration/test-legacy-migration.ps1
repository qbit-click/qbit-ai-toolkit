...
try {
  Test 'fresh member seeds project-owned AI_CONTEXT header and uninstall preserves it' {
    ...
    $Updated=RunJson @('-Operation','update','-Target',$Repo);Assert ($Updated.Code -eq 0) 'version-only state update failed';$UpdatedState=ReadUtf8 $StatePath|ConvertFrom-Json;Assert ([string]$UpdatedState.installerVersion -eq '1.1.0') 'version-only update did not refresh ownership state'
