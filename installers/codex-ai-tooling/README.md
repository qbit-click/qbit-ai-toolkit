...
Common options are target, profile (`auto`, `generic`, `typescript`, or `rust`), output format (`text` or `json`), non-interactive mode, dry-run, and owned-modified policy (`fail` or `replace`). `replace` applies only to a previously recorded installer-owned file, makes a backup, and never overrides unowned conflicts. `-AdoptMatching` / `--adopt-matching` explicitly adopts exact current payload files. Version 1.1.2 retains the coherent, audited per-path `-MigrateLegacy` / `--migrate-legacy` authorization introduced in 1.1.1, classifies an unrecognized legacy migration consistently as conflict exit code 4 on both host entrypoints, and keeps cross-installer Windows validation portable on hosts without `Get-FileHash`. There is no general force option.
