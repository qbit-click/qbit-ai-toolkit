...
migration_creation_rollback(){ repo=$(new_repo migration-create-rollback); bash "$installer_root/install.sh" --target "$repo" --profile rust --skip-bootstrap --skip-doctor >/dev/null; before=$tmp/migration-create-rollback-before; after=$tmp/migration-create-rollback-after; complete_snapshot "$repo" > "$before"; QBIT_TOOLKIT_TEST_FAIL_AFTER_WRITES=1; export QBIT_TOOLKIT_TEST_FAIL_AFTER_WRITES; if bash "$installer_root/install.sh" --target "$repo" --profile typescript --skip-bootstrap --skip-doctor >/dev/null 2>&1; then unset QBIT_TOOLKIT_TEST_FAIL_AFTER_WRITES; return 1; fi; unset QBIT_TOOLKIT_TEST_FAIL_AFTER_WRITES; complete_snapshot "$repo" > "$after"; cmp -s "$before" "$after"; [ ! -e "$repo/.ai/tooling/language-servers/package.json" ]; bash "$installer_root/verify.sh" --target "$repo" >/dev/null; }
...
scenario 'POSIX profile migration typescript to rust removes stale files' migration_typescript_rust
scenario 'POSIX profile migration typescript to generic removes stale files' migration_typescript_generic
scenario 'POSIX profile migration rust to typescript creates and owns files' migration_rust_typescript
scenario 'POSIX modified stale file blocks migration before mutation' modified_stale_blocks_migration
scenario 'POSIX replace-policy migration backs up and removes modified stale file' modified_stale_force_migration
scenario 'POSIX missing stale file is safely retired during profile migration' missing_stale_blocks_migration
scenario 'POSIX migration DryRun displays removals and writes nothing' migration_dryrun
scenario 'POSIX migration injected failure rolls back complete snapshot' migration_rollback
scenario 'POSIX migration creation failure removes newly created files' migration_creation_rollback
