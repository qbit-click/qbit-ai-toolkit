...
function Get-Sha256([string]$Path){$Stream=[IO.File]::OpenRead($Path);$Hash=[Security.Cryptography.SHA256]::Create();try{return -join ($Hash.ComputeHash($Stream)|ForEach-Object{$_.ToString('x2')})}finally{$Hash.Dispose();$Stream.Dispose()}}
...
try {
  ...
  foreach($Order in @('tooling-context','context-tooling')) {
    ...
    $ContextState=Join-Path $Repo '.qbit/toolkit/installed/ai-context.json';$ToolingState=Join-Path $Repo '.qbit/toolkit/installed/codex-ai-tooling.json';$BeforeContext=Get-Sha256 $ContextState
    & powershell -NoProfile -ExecutionPolicy Bypass -File $Tooling -Operation update -Target $Repo -Profile generic -SkipBootstrap -SkipDoctor|Out-Null;Assert ($LASTEXITCODE -eq 0) "Tooling update failed for $Order";Assert ((Get-Sha256 $ContextState) -eq $BeforeContext) "Tooling update changed Context state for $Order"
    $BeforeTooling=Get-Sha256 $ToolingState;& powershell -NoProfile -ExecutionPolicy Bypass -File $Context -Operation update -Target $Repo|Out-Null;Assert ($LASTEXITCODE -eq 0) "Context update failed for $Order";Assert ((Get-Sha256 $ToolingState) -eq $BeforeTooling) "Context update changed Tooling state for $Order"
